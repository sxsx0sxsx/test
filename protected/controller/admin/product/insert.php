 ...
 /*  print_r($_POST);
    ...
    if(!empty($_POST["check_images"])){ //判断是否有上传图片
      ...
      $year =date('Y',time());
       ...
       $_POST["image"]=json_encode($images); //将保存图片的数组转化为json格式的字符串存储在数据库中
       $_POST["thumb_image"]= json_encode($thumb_images);
       $_POST["water_image"]= json_encode($water_images);
   }
   $_POST["time"]=time();
   $result = insert($_POST,"product");
   if($result>0){
   	  header("location:index.php?admin=admin&m=product&a=index");
   }else{
   	  echo "<script>alert('您新增失败')</script>";
   }
...
view("product/insert","admin",array("cate"=>$cate));
?>